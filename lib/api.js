...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    super();
    this.defaultUrl =
      'wss://eastus-ai-production.openai.azure.com/openai/realtime?api-version=2024-10-01-preview&deployment=gpt-4o-realtime-preview';
    this.url = url || this.defaultUrl;
    this.apiKey = apiKey || null;
    this.debug = !!debug;
    this.ws = null;
    console.log('RealtimeAPI constructor', this.url);
  }
  ...
  async connect({ model } = { model: 'gpt-4o-realtime-preview-2024-10-01' }) {
    ...
    if (!!this.apiKey && this.url === this.defaultUrl) {
      this.url = this.url + `&api-key=${this.apiKey}`;
    }

    this.url = this.url.replace(/^http(s?):\/\//, (_, s) => `ws${s}://`);
    ...
    if (globalThis.document) {
      /**
       * Web browser - connect without API key
       */
      ...
      /**
       * Node.js - server-side connection with API key
       */
      const moduleName = 'ws';
